...
const ManageProject = () => {
  ...
  useEffect(() => {
    const fetchProjectAndProposals = async () => {
      try {
        ...
        const proposalsRes = await axios.get(`http://localhost:5000/api/projects/${id}/proposals`, config);
  ...
  const handleUpdateProposal = async (proposalId, status) => {
    try {
      ...
      await axios.put(`http://localhost:5000/api/proposals/${proposalId}/status`, { status }, config);
      ...
      if (status === 'accepted') {
        await axios.put(`http://localhost:5000/api/projects/${id}`, { status: 'in-progress' }, config);
  ...
  const handleCompleteProject = async () => {
    try {
      ...
      await axios.put(`http://localhost:5000/api/projects/${id}`, { status: 'completed' }, config);
